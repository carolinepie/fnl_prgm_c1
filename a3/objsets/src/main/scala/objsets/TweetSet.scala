...
abstract class TweetSet extends TweetSetInterface {
  ...
  def union(that: TweetSet): TweetSet = {
    that match {
      case Empty() => this
      case NonEmpty(elem, left, right) => incl(elem).union(left).union(right)
    }
  }
...
//  def union(that: TweetSet): TweetSet = that
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("most retweeted of empty set")
...
case class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
      ...
      if (elem.retweets >= left_max.retweets && elem.retweets >= right_max.retweets) elem
      else if (left_max.retweets >= elem.retweets && left_max.retweets >= right_max.retweets) left_max
      else right_max
    }
  }
