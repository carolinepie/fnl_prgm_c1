...
class HuffmanSuite {
  ...
  @Test def `times of a larger string`: Unit =
    new TestTrees {
      assertEquals(List(('a',5), ('b',4), ('r',3), ('k',2), ('d',1)), times(string2Chars("aaaaarbrbrbbkkd")))
    }
  ...
  @Test def `create code tree`: Unit =
    new TestTrees {
      assertEquals("aabbb".toList, decode(t1, encode(t1)("aabbb".toList)))
    }

  @Test def `decode check`: Unit =
    new TestTrees {
      assertEquals("ab".toList, decode(t1, encode(t1)("ab".toList)))
    }

  @Test def `encode check`: Unit =
    new TestTrees {
      assertEquals(encode(t1)("abbab".toList), List[Bit](0, 1, 1, 0, 1))
    }
  ...
  @Test def `convert table test`: Unit =
    new TestTrees {
